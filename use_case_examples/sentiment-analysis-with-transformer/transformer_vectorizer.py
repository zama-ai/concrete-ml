...
class TransformerVectorizer:
    ...
    def text_to_tensor(
        ...
        tokenized_text_X_train = self.tokenizer.batch_encode_plus(texts, return_tensors="pt")[
            "input_ids"
        ]
        ...
        tokenized_text_X_train_split = torch.split(
            tokenized_text_X_train, split_size_or_sections=50
        )
        ...
        for tokenized_x in tokenized_text_X_train_split:
            ...
            output_hidden_states = transformer_model(
                tokenized_x.to(self.device), output_hidden_states=True
            )[1][-1]
