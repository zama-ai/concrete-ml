...
1. In some cases (when doing extreme quantization) PTQ is not sufficient to achieve a decent final model accuracy. **Concrete-ML** offer the possibility for the user to do quantization before compilation to FHE; notably, the quantization is completely controlled by the user, and can be done by any means, including by using third-party frameworks. In this approach, the user is responsible for implementing a full-integer model respecting the [FHE constraints](fhe_constraints.md). **Concrete-ML** now supports the compilation of models generated with Quantization Aware Training. Please refer to the [FHE-friendly models documentation](fhe_friendly_models.md)
