...
- [The Virtual Library](#what-is-the-virtual-lib)
- [How to do compilation error analysis](#compilation-error-analysis)
- [How to analyze computational complexity](#computational-complexity-analysis)

The methods described here rely on **Concrete-Numpy** features such as the Virtual Lib.
...
Let's examine a neural network that is not FHE compatible:
...
The above compilation call raises an error such as:
...
We thus determine that the error is in the second layer. Reducing the number of neurons in this layer
will make the network FHE compatible:
...
In FHE, Table Lookup operations are implemented with Programmable Bootstrapping. This operation is
very costly, with respect to the cost of 'levelled' operations, i.e. arithmetic operations
(matrix multiplication, convolution, addition). Furthermore, the cost of a PBS will depend on the
bitwidth of the compiled circuit. Every additional bit in the maximum bitwidth raises the complexity
of the PBS by a significant factor. It thus may be of interest to the model developer to
determine the bitwidth of the circuit and the number of PBS it performs. We can do this by
inspecting the MLIR code produced by **Concrete-Numpy**:
...
We notice that we have calls to `FHELinalg.apply_mapped_lookup_table` and `FHELinalg.apply_lookup_table`.
These calls apply PBS to the cells of their input tensors. Their inputs in the listing above are:
`tensor<1x2x!FHE.eint<8>>` for the first and last call and `tensor<1x50x!FHE.eint<8>>` for the two calls
in the middle. Thus PBS is applied 104 times.

To get the bitwidth of the circuit we can do:
...
Which for this circuit gives 8 bits. Decreasing the number of bits and the number of PBS
induces large reductions in the computation time of the compiled circuit.
