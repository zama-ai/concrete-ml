...
CN_VERSION_SPEC_FOR_RC="concrete-numpy==0.8.0-rc2"
...
check_links:
	...
	@# while sphinx produces html files that are linked. We check only links that work both
	@# in sphinx and gitbook, so ignore the generate lazydocs API doc 
	@# thus generated gitbook API doc links should be checked by linkcheckmd above
